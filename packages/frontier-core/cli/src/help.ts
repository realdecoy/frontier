...
import { log } from '../../../frontier-plugins/plugin-vue/src/lib/stdout';
...
export default class MyHelpClass extends Help {
  ...
  async showRootHelp(): Promise<void> {
    ...
    this.sortedCommands.forEach(command => {
      const numOfSpaces = maxSpaces - command.id.length;
      formattedCommandHelp += `\n\t    ${command.id}${new Array(numOfSpaces + 1).join(' ')} - ${command.description?.slice(0, 30)}`;
    })
