  ...
  if (isExistingFile === false) {
    throw new Error(
      JSON.stringify({
        ...
        message: `template file not found, run ${chalk.whiteBright('mobile upgrade')} to continue`,
      }),
    );
  }
...
async function readAndUpdateFeatureFiles(
  ...
  kebabName: string,
  pascalName: string,
  camelName: string | undefined = undefined,
): Promise<void> {
  let filePath = '';
...
/**
 * Description: determine if is root directory of mobile project
  ...
  if (isExistingFolder === false) {
    throw new Error(
      JSON.stringify({
        ...
        message: `template folder not found, run ${chalk.whiteBright('mobile upgrade')} to continue`,
      }),
    );
  }
}
...
function parseDynamicObjects(
  ...
  // 1[c] Once everything is clear write the updated file into the ./mobile foldler
  if (
